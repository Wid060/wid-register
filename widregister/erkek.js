...
widuye: widmember.id,
isim: nick,
role: config.man_role,
role2: config.man_role2,
...
widmember.roles.cache.has(config.booster_role) ? widmember.roles.set([config.booster_role,  config.man_role, config.man_role2]) : widmember.roles.set([config.man_role, config.man_role2]);
...
const logwid = new MessageEmbed()
  ...
  .addField(`Verilen Roller`,`[<@&${config.man_role}> | <@&${config.man_role2}>]`,true)
  ...
  lgwid.send(logwid)
  krank.add(`erkekTeyit.${message.author.id}`, 1);
